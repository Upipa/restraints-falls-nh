// Bivariate Binomial Type II — facility intercepts + common time slope
//
// logit(p_ij) = beta_j + alpha * t_i
//   beta_j: facility-specific intercept (j = 1..J)
//   alpha:  common time slope across all facilities
//   q, r:   global fall probabilities (restrained / not restrained)
//
// Prior: beta_j, alpha ~ Normal(0, 10) independent; q, r Uniform(0,1) implicit.
//
...
parameters {
  ...
  real alpha;                    // common time slope
  real<lower=0, upper=1> q;     // P(falls | restrained)
  real<lower=0, upper=1> r;     // P(falls | not restrained)
}

model {
  // Priors — all explicit lpmf/lpdf to retain normalizing constants (required for Bayes factor)
  target += normal_lpdf(beta | 0, 10);
  target += normal_lpdf(alpha | 0, 10);

  // Likelihood — both observed
  for (i in 1:N_both) {
    real p_i = inv_logit(beta[ente_both[i]] + alpha * t_both[i]);
  ...
  for (i in 1:N_x_only) {
    real p_i = inv_logit(beta[ente_x_only[i]] + alpha * t_x_only[i]);
    target += binomial_lpmf(x_only[i] | n_x_only[i], p_i);
  }

  // Likelihood — only falls
  for (i in 1:N_y_only) {
    real p_i = inv_logit(beta[ente_y_only[i]] + alpha * t_y_only[i]);
