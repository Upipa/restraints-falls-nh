// Bivariate Binomial Type II — facility intercepts + quadratic time trend
//
// logit(p_ij) = beta_j + alpha * t_i + gamma * t_i^2
//   beta_j: facility-specific intercept (j = 1..J)
//   alpha:  common linear time coefficient
//   gamma:  common quadratic time coefficient
//   q, r:   global fall probabilities (restrained / not restrained)
//
// Prior: beta_j, alpha, gamma ~ Normal(0, 10) independent; q, r Uniform(0,1) implicit.
//
...
parameters {
  ...
  real alpha;                    // common linear time coefficient
  real gamma;                    // common quadratic time coefficient
  real<lower=0, upper=1> q;     // P(falls | restrained)
  real<lower=0, upper=1> r;     // P(falls | not restrained)
}

model {
  // Priors — all explicit lpdf to retain normalizing constants (required for Bayes factor)
  ...
  for (i in 1:N_both) {
    real t = t_both[i];
    real p_i = inv_logit(beta[ente_both[i]] + alpha * t + gamma * t^2);
  ...
  for (i in 1:N_x_only) {
    real t = t_x_only[i];
    real p_i = inv_logit(beta[ente_x_only[i]] + alpha * t + gamma * t^2);
  ...
  for (i in 1:N_y_only) {
    real t = t_y_only[i];
    real p_i = inv_logit(beta[ente_y_only[i]] + alpha * t + gamma * t^2);
