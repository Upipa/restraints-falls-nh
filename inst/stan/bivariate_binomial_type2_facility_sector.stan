// Bivariate Binomial Type II — facility×sector intercepts + common time slope
//
// logit(p_ijk) = beta_k + alpha * t_i
//   beta_k: intercept for facility×sector pair k (k = 1..K)
//   alpha:  common time slope across all facility×sector units
...
//
// Prior: beta_k, alpha ~ Normal(0, 10) independent (fully normalized);
...
parameters {
  array[K] real beta;            // facility×sector intercepts on logit scale
...
model {
  ...
  for (i in 1:N_both) {
    real p_i = inv_logit(beta[fac_sec_both[i]] + alpha * t_both[i]);
  ...
  for (i in 1:N_x_only) {
    real p_i = inv_logit(beta[fac_sec_x_only[i]] + alpha * t_x_only[i]);
    target += binomial_lpmf(x_only[i] | n_x_only[i], p_i);
  }

  // Likelihood — only falls
  for (i in 1:N_y_only) {
    real p_i = inv_logit(beta[fac_sec_y_only[i]] + alpha * t_y_only[i]);
